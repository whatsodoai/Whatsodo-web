...
export default function LeadDetailPage() {
  ...
  return (
    <div className="page-container">
      <Link
        href="/leads"
        className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-200 mb-2 w-fit"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="card p-6">
            <div className="flex items-center justify-end mb-2">
              <button onClick={openEdit} className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-300">
                <Edit2 size={12} /> Edit
              </button>
            </div>
            <div className="flex flex-col items-center text-center mb-5">
              ...
              <div className="mt-3 relative">
                ...
                {statusOpen && (
                  <div className="absolute left-1/2 -translate-x-1/2 top-full mt-1.5 bg-surface-900 border border-white/10 rounded-xl shadow-xl z-10 py-1 min-w-[180px]">
                    {LEAD_STATUSES.map((s) => (
                      <button
                        key={s}
                        onClick={() => handleStatusChange(s)}
                        className={cn(
                          'w-full text-left px-3.5 py-2 text-sm text-gray-200 hover:bg-white/5 flex items-center gap-2.5',
                          lead.status === s && 'bg-white/5 font-medium'
                        )}
                      >
                        <span
              ...
              {[
                ...
                <div key={label} className="flex items-start gap-3 py-2 border-b border-white/5">
                  ...
                  <div>
                    ...
                    {href ? (
                      <a href={href} className="text-gray-100 font-medium hover:text-green-400">
                        {value}
                      </a>
                    ) : (
            ...
            {lead.notes && (
              <div className="mt-4 p-3.5 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                ...
                <p className="text-sm text-amber-200">{lead.notes}</p>
              </div>
            )}
          </div>
          ...
          <div className="card p-4">
            ...
            <div className="space-y-2">
              <a
                href={`tel:${lead.phone}`}
                className="flex items-center gap-3 p-3 rounded-xl bg-blue-500/10 text-blue-300 hover:bg-blue-500/20 transition-colors text-sm font-medium"
              >
                <Phone size={15} /> Call {lead.name}
              </a>
              <Link
                href="/inbox"
                className="flex items-center gap-3 p-3 rounded-xl bg-green-500/10 text-green-300 hover:bg-green-500/20 transition-colors text-sm font-medium"
              >
                <MessageSquare size={15} /> Open Chat
              </Link>
              <Link
                href="/appointments"
                className="flex items-center gap-3 p-3 rounded-xl bg-orange-500/10 text-orange-300 hover:bg-orange-500/20 transition-colors text-sm font-medium"
              >
                ...
                {leadAppointments.map((apt) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-green-400 font-medium">{apt.status}</span>
                    </div>
        ...
        <div className="lg:col-span-2 card flex flex-col" style={{ maxHeight: '75vh' }}>
          <div className="p-5 border-b border-white/10 flex items-center gap-3">
            ...
            <h3 className="font-semibold text-gray-100">Conversation History</h3>
                  ...
                  <div
                    className={cn(
                      ...
                      msg.direction === 'outgoing'
                        ...
                        : 'chat-bubble-in text-gray-100 border border-white/10'
                    )}
                  >
      ...
      {editOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-surface-900 border border-white/10 rounded-2xl w-full max-w-md shadow-2xl animate-slide-up">
            <div className="flex items-center justify-between p-5 border-b border-white/10">
              <h2 className="text-lg font-bold text-gray-100">Edit Lead</h2>
              <button onClick={() => setEditOpen(false)} className="p-1.5 rounded-lg text-gray-500 hover:bg-white/5">
                <X size={18} />
              </button>
            </div>

...
const STATUS_BADGE: Record<Campaign['status'], { bg: string; color: string; label: string }> = {
  ...
  sending: { bg: 'bg-blue-100', color: 'text-blue-600', label: 'Sending' },
          ...
          {templates.length === 0 ? (
            ...
            <>
              ...
              {selectedTemplate?.bodyPreview && (
                <div className="p-3 bg-gray-50 rounded-xl text-xs text-gray-600">{selectedTemplate.bodyPreview}</div>
              )}
              ...
              <div key={c._id} className="flex items-center gap-4 p-4">
                ...
                <div className="flex items-center gap-4 text-xs text-gray-500">
                  <span className="flex items-center gap-1">
                    <CheckCircle size={12} className="text-green-500" /> {c.stats.sent} sent
                  </span>
                  {c.stats.failed > 0 && (
                    <span className="flex items-center gap-1">
                      <XCircle size={12} className="text-red-500" /> {c.stats.failed} failed
                    </span>
                  )}
